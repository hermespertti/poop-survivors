...
// ---------- M13: new enemies (2) — tanky + knockback-resistant variants ----------
// boulder (12x12) — a big armored gunk boulder: high HP, takes reduced knockback.
// 3 brown body, 6 dark armor spots, 5 highlights, 4 outline. 2 wobble frames.
const boulderFrames: string[][] = [
  [
    '....4444....',
    '..44333344..',
    '.4336633634.',
...
const boulder = mk(12, 12, boulderFrames);
const boulderHit = mk(12, 12, boulderFrames.map((f) => f.map((row) =>
  row.split('').map((c) => (c === '3' ? '5' : c === '6' ? 'e' : c === '5' ? 'c' : c === '4' ? '5' : c)).join(''))));
// shell (12x12) — a spiked gunk shell: medium HP, resists knockback (spikes).
// a blue body, 5 highlights, 4 outline/spikes. 2 frames (spike wobble).
...
const shell = mk(12, 12, shellFrames);
const shellHit = mk(12, 12, shellFrames.map((f) => f.map((row) =>
  row.split('').map((c) => (c === 'a' ? '9' : c === '4' ? '5' : c === '5' ? 'c' : c)).join(''))));

// ---------- M13: two new characters (12x12, 2 frames, eyes right of center so
// the M13 facing-flip reads as a direction change) ----------
// cheese — a little gold cheese block with holes; start weapon: mine.
const cheeseFrames: string[][] = [
  [
    '....4444....',
    '..44222244..',
    '.4222222224.',
...
const cheese = mk(12, 12, cheeseFrames);
const cheeseHit = mk(12, 12, cheeseFrames.map((f) => f.map((row) =>
  row.split('').map((c) => (c === '2' ? '5' : c === '3' ? 'e' : c === '5' ? 'c' : c === '4' ? '5' : c)).join(''))));
// onion — a purple onion with a sprout; start weapon: bouncy.
const onionFrames: string[][] = [
  [
    '.....44.....',
    '....4884....',
    '...4bbbb4...',
...
const onion = mk(12, 12, onionFrames);
...
// trail — a slinking slime trail (green blobs, dark green edge, trailing L).
const trail = mk(8, 8, [
  [
    '........',
    '........',
...
// boomer — a gunk boomerang (angular V, gold body, brown shade).
const boomer = mk(8, 8, [
  [
    '2.......',
    '23......',
...
// M13: horizontal mirror of drawSprite (same bounding box — column x is drawn
// at px + (w-1-x)). Characters face their movement/target direction by
// flipping left/right: the sprites' eyes sit right of center, so the flip
// reads as a direction change at 320x240.
export function drawSpriteFlipped(
  ...
  for (let y = 0; y < spr.h; y++) {
    ...
    for (let x = 0; x < spr.w; x++) {
      ...
      if (idx < 0) continue;
      ctx.fillStyle = PALETTE[idx];
      ctx.fillRect(px + (spr.w - 1 - x), py + y, 1, 1);
    }
  }
}
