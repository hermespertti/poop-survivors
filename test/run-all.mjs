...
//   2. Runs the four suites (m1..m4) IN PARALLEL. Parallel is safe here: every
//      bot drives the game through batched in-page step() (deterministic,
...
//   3. Streams each suite's output with a [mN] prefix and prints a summary.
//   4. Exits non-zero if any suite fails.
//
// Wall time (measured, this box): ~5-6 min — dominated by m4's four 10-minute
// character runs. m1/m2/m3 each finish in well under a minute.
...
console.log(`\nGATE: ${totalPass} pass / ${totalFail} fail across ${SUITES.length} suites in ${secs}s — ${bad === 0 ? 'GREEN' : bad + ' suite(s) FAILED'}`);
killIfOwned();
